...
import { getPositionChoices } from '../../../../app/lib/reservation';
...
export const RegisterModal = ({ visible, setVisible, slot, reservations, setReservations, client }) => {
    ...
    const [splitOptions, setSplitOptions] = useState(false);
    ...
    const onSubmit = async e => {
        ...
        try {
            ...
            const allOptionIris = clientWithOptions(client) && isDefinedAndNotVoid(selectedOptions) ? selectedOptions.map(o => o['@id']) : [];
            for (let i = 0; i < quantite; i++) {
                let irisForThis = [];
                if (splitOptions && allOptionIris.length > 0) {
                    irisForThis = allOptionIris.filter((_, idx) => idx % quantite === i);
                } else {
                    irisForThis = i === 0 ? allOptionIris : [];
                }
                const optsForPrice = irisForThis.map(iri => selectedOptions.find(o => o['@id'] === iri)).filter(Boolean);
                const prix = getFinalPrice(selectedCircuit, optsForPrice, selectedOriginContact);
                const newReservation = await dataProvider.create('reservations', {data: {...reservation, selectedOptions: irisForThis, prix}});
    ...
    const onSubmitConversion = async e => {
        ...
        try {
            ...
            const prepaymentOptions = selectedPrepayment.selectedOptions || [];
            const optionIris = clientWithOptions(client) && isDefinedAndNotVoid(prepaymentOptions)
                ? prepaymentOptions.map(o => typeof o === 'string' ? o : o['@id']).filter(Boolean)
                : [];
            const prix = getFinalPrice(circuit, prepaymentOptions, origine);
            const data = {
                    telephone: telephone ?? '',
                    email: email ?? '',
                    quantite: quantite ?? 1, 
            ...
            for (let i = 0; i < data.quantite; i++) {
                const newReservation = await dataProvider.create('reservations', {data: {...data, selectedOptions: optionIris, prix }});
                newReservations = !isDefined(newReservation.data) || (isDefined(newReservation.data.statut) && newReservation.data.statut.includes('CANCEL')) ? 
                newReservations : [...newReservations, newReservation.data];
            }
    ...
    const getFormattedUsedPrepayment = prepayment => {
        ...
        return {
            ...
            selectedOptions: isDefinedAndNotVoid(selectedOptions) ? selectedOptions.map(o => typeof o === 'string' ? o : o['@id']).filter(Boolean) : [],
    ...
    const getFinalPrice = (selectedCircuit, selectedOpts, selectedOriginContact) => {
        ...
        return (isDefined(selectedCircuit) && isDefined(selectedCircuit.prix) ? selectedCircuit.prix : 0) * (1 - (maxOriginDiscount / 100)) + optionsTotal;
    };
                                            ...
                                            <div className="space-y-4 min-h-[543px]">
                                                ...
                                                { clientWithOptions(client) && 
                                                    <OptionForm 
                                                        ...
                                                        quantite={ parseInt(consumer.quantite) || 1 }
