...
export const InformationsModal = ({ selectedReservation, setSelectedReservation, reservations, setReservations, setToUpdate, isSmall }) => {
    ...
    const canEditAgenda = canWrite('agenda');
            ...
            <div id="inside-information-modal" className="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div className="p-4 md:p-5">
                        <div className="space-y-4">
                            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                ...
                                { isDefinedAndNotVoid(selectedReservation.selectedOptions) 
                                    ? <><i>{ selectedReservation.selectedOptions.map(o => o.nom).join(', ') }</i><br/></>
                                    : isDefined(selectedReservation.option) ? <><i>{ selectedReservation.option.nom }</i><br/></> : <></> 
                                }
                                { getResourceInformations() }
                                { selectedReservation.notificationSent && 
                                    <><br/><span style={{ fontSize: '0.7rem', background: '#4caf50', color: 'white', padding: '2px 6px', borderRadius: '4px' }}>✉ Notifié</span></>
                                }
                            </p>
                    ...
                    <div className="flex justify-between items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button onClick={ e => onUpdate() } data-modal-hide="default-modal" type="button" className="text-white bg-blue-700 disabled:bg-blue-800 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 dark:disabled:ring-blue-800"
                                disabled={!isDefined(session) || !isDefined(user) || !canEditAgenda}
                        >
                            Modifier
                        </button>
