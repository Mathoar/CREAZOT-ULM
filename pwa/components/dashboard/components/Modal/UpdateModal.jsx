...
export const UpdateModal = ({ toUpdate, setToUpdate, reservations, setReservations, client }) => {
    ...
    const onSubmit = async e => {
        ...
        try {
            const prix = getFinalPrice(selectedCircuit, selectedOptions, selectedOriginContact);
            const optionIris = clientWithOptions(client) && isDefinedAndNotVoid(selectedOptions) ? selectedOptions.map(o => o['@id']) : [];
    ...
    const getFinalPrice = (selectedCircuit, selectedOpts, selectedOriginContact) => {
        ...
        const optionsTotal = Array.isArray(selectedOpts) ? selectedOpts.reduce((sum, o) => sum + (o?.prix || 0), 0) : (selectedOpts?.prix ?? 0);
        return (selectedCircuit?.prix ?? 0) * (1 - (maxOriginDiscount / 100)) + optionsTotal;
    };
                        ...
                        <form className="space-y-4">
                            ...
                            { clientWithGroupUpdate(client) && section !== "options" &&
                                <div className="flex items-center justify-end space-x-1">
                                    <label className={`text-xs italic text-teal-700 ' ${isDefined(consumer.quantite) && consumer.quantite <= 1 ? 'line-through decoration-teal-700 decoration-1' : ''}>`}>Appliquer au groupe</label>
                                    <Checkbox checked={ applyToGroup } onChange={() => setApplyToGroup(!applyToGroup) } disabled={ isDefined(consumer.quantite) && consumer.quantite <= 1 } />
                                </div>
                            }
                            <div className="border-t border-gray-200 rounded-b dark:border-gray-600">
                                <div className="flex justify-between items-center pt-3 pb-4 md:pb-5">
                                    ...
                                    <button onClick={ onSubmit } disabled={ !isNotBlank(selectedCircuit) } className="text-white bg-green-700 hover:bg-green-800 disabled:bg-green-900 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 disabled:cursor-default cursor-pointer">
