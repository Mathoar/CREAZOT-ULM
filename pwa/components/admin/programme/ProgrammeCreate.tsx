...
const transformProgramme = (data: any) => ({
  ...
  programmeLessons: (data.programmeLessons || []).map((pl: any) => ({
    ...pl,
    lesson: typeof pl.lesson === 'object' ? pl.lesson['@id'] : pl.lesson,
  })),
});

export const ProgrammeCreate = () => (
  <Create transform={transformProgramme}>
    <SimpleForm>
      <TextInput source="nom" label="Nom du programme" validate={required()} fullWidth />
      <SelectInput source="type" label="Type de formation" choices={programmeTypes} defaultValue="brevet" validate={required()} />
      ...
      <ArrayInput source="programmeLessons" label="">
        <SimpleFormIterator inline disableReordering={false}>
          <ReferenceInput source="lesson" reference="lessons" filter={{ isAvailable: true, pagination: false }}>
            <AutocompleteInput
          ...
          <FormDataConsumer>
            {({ scopedFormData }) => {
              const lessonIri = scopedFormData?.lesson;
