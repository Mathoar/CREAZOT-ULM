...
const formatSelectedOptions = (record) => {
  if (!record) return '';
  if (isDefinedAndNotVoid(record.selectedOptions)) {
    return record.selectedOptions
      .map(o => (typeof o === 'object' ? o?.nom : null))
      .filter(Boolean)
      .join(', ');
  }
  return '';
...
const OptionField = ({ client }) => {
  return !clientWithOptions(client) ? null :
    <FunctionField label="Options" sortable={false} render={formatSelectedOptions}/>
};
    ...
    >
        { isSmall ? 
            <SimpleList
              ...
              tertiaryText={ record => {
                const opts = formatSelectedOptions(record);
                return record.circuit.code + (opts ? ' + ' + opts : ' ');
              }}
              rowSx={ ColoredRowSx }
              linkType="show"
            /> 
