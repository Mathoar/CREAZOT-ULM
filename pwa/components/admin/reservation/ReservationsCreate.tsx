...
useDataProvider, BooleanInput, useCreate, useRedirect, useNotify, ReferenceArrayInput, AutocompleteInput} from "react-admin";
...
const OptionInput = ({ client }) => {
  if (!clientWithOptions(client)) return null;
  const quantite = useWatch({ name: 'quantite' });
  const selectedOptions = useWatch({ name: 'selectedOptions' });
  const showSplit = parseInt(quantite) > 1 && Array.isArray(selectedOptions) && selectedOptions.length > 1;
  return (
    <>
      <ReferenceArrayInput source="selectedOptions" reference="options" label="Options" filter={{ isAvailable: true }} />
      { showSplit && <BooleanInput source="splitOptions" label="Répartir les options sur chaque vol" defaultValue={false} /> }
    </>
  );
}
...
export const ReservationsCreate = () => {
  ...
  const getOptions = () => {
    dataProvider
        .getList("options", {})
        .then(({ data }) => setOptions(data.filter(o => o.isAvailable !== false)));
  };

  const onSubmit = async ({selectedOptions: selOpts, splitOptions: doSplit, origine, contact, remarques, quantite, pilote, avion, position, ...data}) => {
    ...
    try {
      ...
      const allOpts = clientWithOptions(client) && isDefinedAndNotVoid(selOpts) ? selOpts.map(o => getFormattedValueForBackEnd(o)) : [];
      const baseData = {
        ...data,
        quantite,
      ...
      for (let i = 0; i < quantite; i++) {
        let optsForThis = [];
        if (doSplit && allOpts.length > 0) {
          optsForThis = allOpts.filter((_, idx) => idx % quantite === i);
        } else {
          optsForThis = i === 0 ? allOpts : [];
        }
        const optsForPrice = optsForThis.map(iri => options.find(o => o['@id'] === iri)).filter(Boolean);
        const prix = getTotalPrice(selectedCircuit, optsForPrice, selectedOrigines);
        await create('reservations', {data: {...baseData, selectedOptions: optsForThis, prix}});
      }
  ...
  const getTotalPrice = (circuit, selectedOpts, origines) => {
      ...
      return (isDefined(circuit) && isDefined(circuit.prix) ? circuit.prix : 0) * (1 - (maxOriginDiscount / 100)) + optionsTotal;
  };
