...
const OptionInput = ({ client }) => !clientWithOptions(client) ? null : 
    <ArrayInput source="selectedOptions" label="Options">
      <SimpleFormIterator inline disableReordering>
        <ReferenceInput reference="options" source="@id" filter={{ isAvailable: true }}>
          <SelectInput label="Option" optionText="nom" />
        </ReferenceInput>
      </SimpleFormIterator>
    </ArrayInput>
...
const OriginContactInput = ({ client }) => { 
    ...
    return <ReferenceArrayInput source="contacts" reference="contacts" label="Canal de contact" filter={{ client: client?.id }}/>;
}
...
export const ReservationsEdit = () => {
  ...
  const transform = ({circuit, selectedOptions: selOpts, debut, avion, pilote, contacts, origineIds, cadeau, paid, option, ...data}) => {
    const selectedCircuit = isDefined(circuit) ? circuits.find(c => c['@id'] === getFormattedValueForBackEnd(circuit)) : null;
    const selectedOpts = clientWithOptions(client) && isDefinedAndNotVoid(selOpts) ? selOpts.map(o => getFormattedValueForBackEnd(o)) : [];
    const selectedOptsForPrice = selectedOpts.map(iri => options.find(o => o['@id'] === iri)).filter(Boolean);
    ...
    return {...data,
        ...
        prix: getTotalPrice(selectedCircuit, selectedOptsForPrice, selectedOrigines),
