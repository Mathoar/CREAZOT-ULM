    ...
    >
        { isSmall ? 
            <SimpleList
              ...
              tertiaryText={ record => {
                const hasNewOptions = Array.isArray(record?.selectedOptions) && record.selectedOptions.length > 0;
                const hasLegacyOption = isDefined(record?.option) || isDefined(record?.options);
                return (isDefined(record.quantite) ? record.quantite : 1) + " x " + (record?.circuit?.code ?? '') + ((hasNewOptions || hasLegacyOption) ? ' + options' : '');
              }}
              linkType="show"
            /> 
            : 
